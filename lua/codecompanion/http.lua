...
---Send an AWS-signed HTTP request for Bedrock
---@param payload { messages: table, tools: table|nil } The payload to be sent to the endpoint
---@param actions CodeCompanion.HTTPAdapter.RequestActions
---@param opts? table Options that can be passed to the request
---@param aws_config table AWS credentials and region
---@return table|nil The Plenary job
function Client:request_with_aws_signing(payload, actions, opts, aws_config)
  ...
  local url = adapter_utils.set_env_vars(adapter, adapter.url)
  local headers = adapter_utils.set_env_vars(adapter, adapter.headers)
  
  local signed_headers = aws_signing.sign_request({
    method = "POST",
    url = url,
    headers = headers,
    payload = body,
  ...
  if body_file then
    request_opts["body"] = "@" .. body_file.filename
  end

  if actions.done then
    request_opts["callback"] = self.methods.schedule_wrap(function(response)
      cleanup("success")
      actions.done()
      cb(nil, response, adapter)
    end)

    request_opts["on_error"] = self.methods.schedule_wrap(function(err)
      cleanup("error")
      cb(err, nil, adapter)
    end)
  else
  ...
  if adapter.opts and adapter.opts.raw then
    if type(adapter.opts.raw) == "table" then
      vim.list_extend(request_opts.raw, adapter.opts.raw)
    else
      table.insert(request_opts.raw, adapter.opts.raw)
    end
  end

  if adapter.opts and adapter.opts.compress then
    request_opts["compressed"] = adapter.opts.compress or false
